...
### 10. AI Agent Command Execution Security

When building AI agents that execute shell commands, implement strict validation:
    ...
    for segment in segments:
        ...
        for token in tokens:
            if token in ("|", "||", "&&", "&"):
                expect_command = True
                continue
            if token in ("if", "then", "else", "fi", "for", "while"):
                continue
...
def validate_pkill_command(command_string: str) -> tuple[bool, str]:
    ...
    allowed = {"node", "npm", "npx", "vite", "next"}
    ...
    if target in allowed:
        return True, ""
    return False, f"pkill only allowed for: {allowed}"


def validate_chmod_command(command_string: str) -> tuple[bool, str]:
    """Only allow making files executable."""
    tokens = shlex.split(command_string)
    mode = None
    ...
    if not re.match(r"^[ugoa]*\+x$", mode or ""):
...
- [ ] Complex command parsing (pipes, chaining) handled
- [ ] Fail-safe: block on parsing errors
- [ ] Additional validation for sensitive commands
- [ ] Security hooks integrated with agent SDK
